...
** Emacs daemon
   ...
   #+BEGIN_SRC conf
   exec emacs --daemon && emacsclient -e "(my-org-agenda)"
   #+END_SRC
** Safe Eyes
   #+BEGIN_SRC conf
   exec safeeyes
...
** udiskie
   ...
   #+END_SRC
** kanshi
   Start kanshi (needed for dynamic display configuration):
   #+BEGIN_SRC conf
   exec kanshi
   #+END_SRC
* Key bindings
** Basics
   ...
   open =org-agenda=
   #+BEGIN_SRC conf
       bindsym $mod+a exec emacsclient -ce '(switch-to-buffer "*Org Agenda*")'
   #+END_SRC
...
** Fn keys
   Create fifo for =wob=:
   #+BEGIN_SRC conf
   exec mkfifo $SWAYSOCK.wob && tail -f $SWAYSOCK.wob | wob --border-color '<<base0D>>FF' --background-color '<<base00>>FF' --bar-color '<<base07>>FF'
   #+END_SRC
