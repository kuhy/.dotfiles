...
** Music player daemon
Run music player in background so I can send commands to it:
#+BEGIN_SRC conf
exec tmux new-session -d -s <<music_player>> <<music_player>>
#+END_SRC
...
#+BEGIN_SRC conf
    bindsym $mod+f1 exec <<menu>>
#+END_SRC

open file from home directory
#+BEGIN_SRC conf
    bindsym $mod+f2 exec <<opener>> "$(find ~ | rofi -dmenu -i -p 'find')"
#+END_SRC
...
start emacs
#+BEGIN_SRC conf
    bindsym $mod+e $open emacs
#+END_SRC

attach tmux session with music player
#+BEGIN_SRC conf
    bindsym $mod+m exec <<terminal>> -e 'tmux attach-session -t <<music_player>>'
#+END_SRC
